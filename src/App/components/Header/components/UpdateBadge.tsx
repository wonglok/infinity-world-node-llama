...
const latestReleaseUrl =
    "https://github.com/withcatai/node-llama-cpp/releases/latest";
...
export function UpdateBadge({
    ...
    const nextUpdateTimeoutRef = useRef<
    ...
    const updateLatestVersionInfo = useCallback(
        async function updateLatestVersionInfo() {
            clearTimeout(nextUpdateTimeoutRef.current);
            await withLock(
                [instanceLock.current, "updateVersion"],
                async () => {
                    clearTimeout(nextUpdateTimeoutRef.current);

                    const latestVersion = await getLatestAvailableVersion(
                        appVersionIsBeta ?? false,
                    );
                    if (
                        shouldUpdateCurrentVersion.current &&
                    ...
                    nextUpdateTimeoutRef.current = setTimeout(
    ...
    return (
        <div className="updateBadge">
            {!releasedVersionIsNewerThanCurrent &&
                appVersion &&
                canShowCurrentVersion && (
            ...
            {releasedVersionIsNewerThanCurrent && releaseLink != null && (
                <a target="_blank" href={releaseLink} className="newVersion">
                    Version <code className="version">{latestVersion}</code> is
                    available
                </a>
            )}
        </div>
...
async function getLatestAvailableVersion(
    ...
    try {
        if (includePrerelease) {
            ...
            if (
                latestReleases.latestPrerelease != null &&
                latestReleases.latestRelease != null
            ) {
                if (
                    semver.gt(
                        latestReleases.latestPrerelease.version,
                        latestReleases.latestRelease.version,
                    )
                )
                    return {
