...
export function InputRow({
    ...
    return (
        <>
            ...
            `}</style>
            <FixedDivWithSpacer
                className={`input-row flex flex-row bottom-4 shrink-0 items-end w-[calc(100%-32px)] max-w-(--app-max-width) bg-(--panel-background-color) rounded-[1.25rem] shadow-(--panel-box-shadow) overflow-clip text-(--panel-text-color) border-2 border-[rgba(200,170,120,0.2)] ${disabled ? "opacity-[0.48]" : ""}`}
            >
                <div className="flex-1 flex flex-row overflow-hidden relative isolate max-h-[400px] input-area">
                    <textarea
                        ...
                        placeholder={
                            autocompleteText === "" ? "Type a message..." : ""
                        }
                    />
                    <div
